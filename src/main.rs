...
fn main() {
    ...
    s1.add_shit(&mut vec![0x01, 0x00, 0x00, 0x00, 0x01, 0x00, 0x00, 0x00]);
    let mut s2 = Scope::start(Some(&mut s1), false, false);
    s2.add_variable(Some("b".to_string()), &mut s1);
    s2.add_shit(&mut vec![0x01, 0x00, 0x00, 0x00, 0x01, 0x00, 0x00, 0x00]);
    s2.add_variable(Some("b".to_string()), &mut s1);
    s2.add_shit(&mut vec![0x01, 0x00, 0x00, 0x00, 0x01, 0x00, 0x00, 0x00]);
    ...
    // s3.add_shit(&mut vec![0x02,0x00,0x00,0x00,0x01,0x00,0x00,0x00]);
    // s3.end();
    s1.end();
